...
class Test(unittest.TestCase):
    def _run(self, GPU):
        ...
        kernel1 = GPR.kern.RBF()
        kernel2 = GPR.kern.Matern52()
        kernel1_summation = GPR.kern.Summation(kernel1)
        kernel2_summation = GPR.kern.Summation(kernel2)
        kernel_addition_summation = GPR.kern.AdditionKernel([kernel1_summation, kernel2_summation])
        gp = GPR.GP(X_train, Y_train, kernel_addition_summation, 1e-4, GPU=GPU)
        ...
        gp.save("model")
        err = np.max(np.abs(GPR.GP.load("model.npz", GPU).predict(X_test) - pred_test))
